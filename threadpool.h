...
public:
	...
	auto submitTask(T &&fun, Args&&...args)->std::future<std::invoke_result_t<T,Args...>>//提交任务(任务一般在堆上建立)
	{
		using RType = std::invoke_result_t<T,Args...>;
		auto task = std::make_shared<std::packaged_task<RType( )>>(std::bind(std::forward<T>(fun),std::forward<Args>(args)...));
		...
		if (state)
		{
			...
			if (notfull.wait_for(lock, std::chrono::seconds(4), [&]()->bool {return Tasksize_ < Taskquemaxsize_; }) == false)
			{
				...
				auto test = std::make_shared<std::packaged_task<RType()>>([&]()->RType {return RType(); });
					std::future<RType> result_x = test->get_future();
				(*test)();
				return result_x;
			}
			que.emplace([task]()->void {(*task)();});
			Tasksize_++;
